			...
			<tbody>
				<template id="coins-cryptofolio-template">
					...
					<tr>
						...
						<td>{{name}}</td>
						<td>{{price}}</td>
						<td><input type="number" value="{{amount}}" id="{{id}}" class="amount-input" /></td>
						<td class="price-total">{{totalValue}}</td>
						<td><button type="button" class="btn btn-warning save-coin-btn" value="{{id}}">Save</button>
						</td>
						<td><button type="button" class="btn btn-danger delete-coin-button" value="{{id}}">Delete</button></td>
					</tr>
					{{/data}}
				</template>
			</tbody>
